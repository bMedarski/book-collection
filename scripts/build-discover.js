...
async function fetchAuthor(name, attempt = 1) {
  const url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent('inauthor:"' + name + '"')}&langRestrict=en&maxResults=40&orderBy=relevance&key=${API_KEY}`;
  const res = await fetch(url);
  if (!res.ok) {
    if ((res.status === 503 || res.status === 429) && attempt < 4) {
      await sleep(500 * attempt);
      return fetchAuthor(name, attempt + 1);
    }
    console.warn(`  HTTP ${res.status} for ${name} (giving up after ${attempt} attempts)`);
    return [];
  }
...
async function main() {
  ...
  for (const [, byTitle] of perAuthorResults) {
    const top = [...byTitle.values()]
      .sort((a, b) => b.ratingsCount - a.ratingsCount || b.rating - a.rating)
      .slice(0, 8);
    books.push(...top);
  }
